...
<head>
	...
	<title>Lestari Collection | Produk</title>
...
?>	
		...
		<div class="container pb-5">
			<div class="produk row">
				...
				<?php  
				$sql = mysqli_query($conn, "SELECT * FROM produk LIMIT 12");
				if(isset($_POST['cari'])) {
					$key = $_POST['keyword'];
					$sql = mysqli_query($conn, "SELECT * FROM produk WHERE nama LIKE '%$key%'");
				}
				if(isset($_GET['kategori'])) {
					$kategori = $_GET['kategori'];
					$sql = mysqli_query($conn, "SELECT * FROM produk WHERE kategori='$kategori'");
				}
				...
				<div class="col-sm-3 text-center">
					<div class="item-produk">
						<a href="detail.php?id=<?= $data['id_produk']; ?>" title="<?= $data['nama'], $data['kode_produk']; ?>"><img src="../images/produk/<?= $data['img']; ?>" width="244" height="356" alt=""></a>
						<span class="text-center text-secondary font-weight-bold"><?= strtoupper($data['nama']); ?></span><br>
						<span class="text-secondary">Rp.<?= number_format($data['harga']); ?></span>
					</div>
				</div>
